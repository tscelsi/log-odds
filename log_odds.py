...
def main(args: argparse.Namespace) -> None:
    ...
    c_1 = _get_dict(args.corpus_1)
    c_2 = _get_dict(args.corpus_2)
    c_bg = _get_dict(args.corpus_bg)
    ...
    if args.out:
        with open(f"{args.out}", "w") as sink:
            sink.write(f"Top {args.lim} from {args.corpus_1}\n")
            for tok, rat in ratios[: args.lim]:
                sink.write(f"{tok}\t{rat}\n")
            sink.write(f"Top {args.lim} from {args.corpus_2}\n")
            for tok, rat in ratios[: -args.lim - 1 : -1]:
                sink.write(f"{tok}\t{rat}\n")
    else:
        print(f"Top {args.lim} from {args.corpus_1}")
        for tok, rat in ratios[: args.lim]:
            print(f"{tok}\t{rat}")
        print(f"\nTop {args.lim} from {args.corpus_2}")
        for tok, rat in ratios[: -args.lim - 1 : -1]:
            print(f"{tok}\t{rat}")
